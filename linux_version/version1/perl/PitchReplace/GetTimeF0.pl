...
open(in,"/home/APP/workspace_perl/PitchReplace/finalset.list")||die;
while(<in>){
	chomp;
	$hash_final{$_} = 1;
}
...
#					print "$_\n";
		...
		for($k = 2; $k < @timeInfo_syllable; $k = $k + 2){
			...
			$timeDeleteOnset = $timeInfo_final[$k]+($timeInfo_final[$k+1]-$timeInfo_final[$k])*0.03; #弯头0.03
			$timeDeleteCoda = $timeInfo_final[$k+1]-($timeInfo_final[$k+1]-$timeInfo_final[$k])*0.05; #降尾0.05
		}	
		close(out);	
		close(in);		
	}		
...
system("/usr/praat/praat --run /home/APP/workspace_praat/GetPitchTierF0.scp $praat_filedir");
open(in,"/home/APP/workspace_perl/PitchReplace/finalset.list")||die;
while(<in>){
	chomp;
	$hash{$_} = 1;
...
for($i = 2; $i < @file_data; $i++) {
	if($file_data[$i] =~ /(.+).PitchTier/){
		...
		open(PitchTier, "$filedir/$file_data[$i]");
		open(PitchTierToF0, ">$filedir/$PitchTierToF0");
