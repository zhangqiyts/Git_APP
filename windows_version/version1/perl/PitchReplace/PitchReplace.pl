sub genconti { 
	...
	system("d:\\usr\\praat\\Praat.exe --run d:\\home\\APP\\workspace_praat\\replaceF0.scp $data $wav $new_wav $startTime_To_endTime");
}
...
$path = "";
@dir_array = split(/\//,$dir);
for($i = 0; $i < @dir_array-1; $i++){
	$path = $path.$dir_array[$i]."\\";
}
$path = $path.$dir_array[@dir_array-1];
...
# $filedir_result = $path."\\".$L2_FileName."\\result";
$workspace_dir = "D:\\workspace_java\\PitchReplace\\WebContent\\data\\";
...
for($i = 2; $i < @file_data; $i++) {
	...
	if($file_data[$i] =~ /(.+)\.wav/ || $file_data[$i] =~ /(.+)\.png/){
		system("copy $file_data_dir\\$file_data[$i]  $filedir_result\\$file_data[$i]");
	}
}
system("copy $file_Chinese_dir\\$Chinese_FileName.wav  $filedir_result\\$Chinese_FileName.wav");
opendir(Dir, $goaldir) || die;
@PitchReplaceWav = readdir(Dir);
for($i = 2; $i < @PitchReplaceWav; $i++){
	if($PitchReplaceWav[$i] =~ /(.+)\_PitchReplace.wav/){
		system("copy $goaldir\\$PitchReplaceWav[$i]  $filedir_result\\$PitchReplaceWav[$i]");
	}
}
closedir(Dir);
