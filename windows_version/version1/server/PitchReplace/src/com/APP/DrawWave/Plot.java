...
final public class Plot {
	 ...
	 */
	public static PlotFrame plot(DrawableObject drawableObject,
			String fileName, int w, int h,boolean _isVisible) {
		if (autoFigure)
			figrue();
		else
			initCurrentPlotFrame();
		...
		p.getPlotPanle().paintImage(fileName, w, h);
	...
	public static PlotFrame plot(double[] x, String pams,String fileName, int w, int h,boolean _isVisible) {
		return plot(new DrawableObject(x, pams),fileName,w,h,_isVisible);
	}
