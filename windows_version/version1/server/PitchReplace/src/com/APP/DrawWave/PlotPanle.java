...
public class PlotPanle extends JPanel {
	...
	//生成图片
	public void paintImage(String fileName,int _w,int _h) {
		w=_w;
		h=_h;
		  ...
		  for (int i = 0; i < drawableObjectStack.size(); ++i) {
				if (drawableObjectStack.get(i).show)
					this.paintDrawableObject(g2, drawableObjectStack.get(i),w,h);
			}
		//	drawPlotInfo(g2);
		  try {
	...
	private void drawLine(Graphics g, double x1, double y1, double x2, double y2) {
		g.drawLine(mappingX(x1), mappingY(y1/2), mappingX(x2), mappingY(y2/2));
	}

	private void drawPoint(Graphics g, double x, double y) {
		g.drawOval(mappingX(x, true), mappingY(y/2, true), attribute.width, attribute.width);
	}
	 ...
	 * 绘制可绘制对象
	 */
	private void paintDrawableObject(Graphics g, DrawableObject drawableObject,int _w,int _h) {
		w=_w;
		h=_h;
		double[] x = drawableObject.x;
		double[] y = drawableObject.y;
	...
	public List<DrawableObject> getDrawableObjectStack() {
		return drawableObjectStack;
	}

	public void setDrawableObjectStack(List<DrawableObject> drawableObjectStack) {
		this.drawableObjectStack = drawableObjectStack;
	}
}
