...
public class Upload extends HttpServlet {
    ...
    private static String PROJECT_DIR = "D:/workspace_java/PitchReplace/WebContent/data/";
    public Upload() {
        super();
    }
    public void destroy() {
...
//                		ToneLearnDetection.ToneDetection(UPLOAD_DIR, L2_ID, wavname); //声调检测
                		...
                		if(DetectionResult.size() == 0 || DetectionTone.size() == 0){
            				score = -1;
            			}else{
            				score = ToneLearnDetection.GetDetectScore(DetectionResult);
            				toneResultFlag = ToneLearnDetection.CompareTone(CorrectTone, DetectionTone);
            				feedbackFlag = ToneLearnDetection.GetFeedBackFlag(toneResultFlag); //0代表存在错误，需要给反馈，1代表全部正确
